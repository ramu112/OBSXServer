...
public void processSimulator() {
  try {
	...
	if(data!=null){	
		...
		List<ProvisioningRequest> provisioningRequests = this.provisioningRequestRepository.findUnProcessedProvisioningRequests();
		...
		if(data.getUpdateStatus().equalsIgnoreCase("Y")){ 
			if(!provisioningRequests.isEmpty()){
				...
				for (ProvisioningRequest provisioningRequest : provisioningRequests) {
					fw.append("simulator Process Request id="+provisioningRequest.getId()+" ,orderId="+provisioningRequest.getOrderId()+" ,Provisiong System="
							+provisioningRequest.getProvisioningSystem()+" ,RequestType="+provisioningRequest.getRequestType()+"\r\n");
		...
		if(data.getcreateTicket().equalsIgnoreCase("Y")){
			...
			for (ProvisioningRequest provisioningRequest : provisioningRequests) {
				Order order=this.orderRepository.findOne(provisioningRequest.getOrderId());
				...
				if(order != null){
				jsonobject.put("description","ClientId"+provisioningRequest.getClientId()+" Order No:"+order.getOrderNo()+" Request Type:"+provisioningRequest.getRequestType()
				...
				this.ticketMasterApiResource.processCreateTicket(provisioningRequest.getClientId(), jsonobject.toString());
			}
		}
	}
