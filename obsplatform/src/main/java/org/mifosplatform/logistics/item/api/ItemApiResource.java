...
public class ItemApiResource {
	...
	private ItemData handleTemplateData() {
		List<SupplierData> supplierData = null;
		...
		final ConfigurationPropertyData configurationData = this.configurationReadPlatformService.retrieveGlobalConfigurationByName("item-supplier-mapping");
		if(configurationData.isEnabled()){
			supplierData = this.supplierReadPlatformService.retrieveSupplier();
		}
		return new ItemData(itemClassdata, unitTypeData, chargeDatas, regionDatas, configurationData , supplierData);
	}
	...
	public String retrieveSingletemData(@PathParam("itemId") final Long itemId, @Context final UriInfo uriInfo) {
		...
		itemData.setConfigurationData(configurationData);
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());
	...
	@Consumes({ MediaType.APPLICATION_JSON })
	@Produces({ MediaType.APPLICATION_JSON })
	public String retrieveSupplierItems(@Context final UriInfo uriInfo,@PathParam("supplierId")final Long supplierId) {
	...
	public String retrieveItemsForDropdown(@Context final UriInfo uriInfo) {
		context.authenticatedUser().validateHasReadPermission(resourceNameForPermissions);
		List<ItemData> itemData = this.itemReadPlatformService.retrieveAllItemsForDropdown();
		final ApiRequestJsonSerializationSettings settings = apiRequestParameterHelper.process(uriInfo.getQueryParameters());	
