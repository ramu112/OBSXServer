       ...
       private static final class parentClientMapper implements RowMapper<ClientCategoryData> {
		...
		private class ClientResourceMapper implements RowMapper<ClientData> {
				...
				return " c.id AS id, c.account_no AS accountNo, c.status_enum AS statusEnum, o.NAME AS officeName ,c.external_id AS externalId, "+
					   " c.display_name AS displayName, b.balance_amount AS balanceAmount, cu.username AS userName, "+
					   " cu.password AS clientpassword, (Select coalesce(min(ba.serial_no), 'No Device') "+
					   " from b_allocation ba where c.id = ba.client_id and ba.status='allocated') HW_Serial "+
			...
			public ClientData mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				return ClientData.instance(accountNo,null, status, null, officeName, id, null, null, null, null, displayName,
	                    externalId, null, null,null,null,null,null, null, null, null, null, null, null,
	                    clientBalance,hwSerial,null,null,null,null,userName,clientpassword,null,null);
			}
		...
		public ClientData retrieveSearchClientId(final String columnName,final String columnValue) {
			try{
				...
				if ((columnName.equals("serial_no")) || (columnName.equals("provisioning_serialno"))) {
					sql = "select client_id AS id from b_item_detail where "+columnName+" = '"+columnValue+"' ";
				} else {
					sql = "select id from m_client where "+columnName+" = '"+columnValue+"' ";
				}
		...
		private static final class SearchMapper implements RowMapper<ClientData> {
	      ...
	      public ClientData mapRow(final ResultSet rs,final int rowNum) throws SQLException {
	          final Long id = rs.getLong("id");
	         return ClientData.searchClient(id);
